...
def timed_inference(prompt: str, base_model, MTP_model, k: int, tokenizer: Tokenizer):
    """
    ...
    print(f"Base Model Time taken: {end_base - start_base:.6f} seconds")
    print("-"*50 + " MTP MODEL " + "-"*50)
    print(f"MTP Model Output: {output_mtp}\n")
    print(f"MTP Model Time taken: {end_mtp - start_mtp:.6f} seconds")
...
def main():
    ...
    timed_inference(prompt=prompt_1, base_model=base_model, MTP_model=mtp_model, k=config.k, tokenizer=tokenizer)
