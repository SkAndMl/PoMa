...
# Model Loading -- Llama: Load to CPU, take the embedding layer and discard the rest
...
embedding_layer = llm_model.tok_embeddings.to(device)
del llm_model

logger.info("Embedding Layer Locked and Loaded!")
...
for epoch in range(config.epochs):
    ...
    for i, batch in enumerate(train_dl):
        ...
        # Embedding layer input -- Padded at the start; hence, last k tokens target
        context_tensor = batch[:, :-config.k].to(device) # shape: (bsz, context_len)
        
        # Forward pass: Get Embeddings
        with torch.no_grad():
            current_input = embedding_layer(context_tensor)  # shape: (bsz, context_len, dim)

        embedding_output = current_input.cpu()
        ...
        seq_length = embedding_output.shape[1] + config.k
        ...
        context_tensor[:, :-config.k, :] = embedding_output
        ...
        while current_step < config.k:
            ...
            with torch.no_grad():
                ...
                final_token_embeddings = embedding_layer(final_token_ids) # shape: (bsz, dim)
    ...
    with torch.no_grad():
        for i, val_batch in enumerate(val_dl):
            ...
            while current_step < config.k:
                ...
                prediction_embeddings = embedding_layer(predicted_ids)  # shape: (batch_size, dim)
